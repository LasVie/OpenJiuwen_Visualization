...
- `allowed / denied / failed` 等 outcome；
- 稳定、无业务原文的 detail code。

受控执行审计只记录 opaque execution id、permission id 和摘要前缀，不记录仓库路径、diff、命令或测试输出。审计也不记录 secret、prompt、Context、Tool 参数/结果、Rail 输入/输出或模型文本。Host 数据库位于项目忽略目录，不进入 Git；Runtime 原文与受控执行原文分别由独立数据库管理。
